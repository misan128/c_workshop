...
/**
 * If list is non-null deallocates all of the nodes on list and sets ir to the null
 * pointer
 * 
...
/**
 * Creates and returns a list. It's called with N non-null pointers followed by one
 * null pointer, and it creates a list with N nodes whose first fields hold the N
 * non-null pointers and whose Nth rest field is null.
 ...
 * @param {function} apply   Function that takes a pointer to a pointer and an
 ...
 * an empty list. If x is non-null, *x is assigned the contents of the first field
 * of the first node
 * 
